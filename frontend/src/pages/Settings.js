...
const Settings = () => {
  ...
  const handlePreferenceSave = () => {
    showToast('System theme preferences applied!');
  };
        ...
        <div className="lg:col-span-4 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-saas overflow-hidden p-3.5 space-y-1.5 shrink-0">
          <button
            ...
            className={`flex items-center gap-3.5 w-full py-3.5 px-4 rounded-xl text-xs font-black tracking-wider uppercase transition-smooth border-l-4 ${
              activeTab === 'account'
                ? 'bg-indigo-50/20 dark:bg-indigo-950/40 border-indigo-500 dark:border-indigo-400 text-indigo-650 dark:text-indigo-400'
        ...
        <div className="lg:col-span-8 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-saas p-6 md:p-8 relative overflow-hidden">
          ...
          <AnimatePresence mode="wait">
            ...
            {activeTab === 'account' && (
              ...
              >
                ...
                <form onSubmit={handleProfileSave} className="space-y-4">
                  ...
                  <button 
                    type="submit"
                    className="py-2.5 px-6 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs tracking-wide transition-smooth shadow-md shadow-indigo-600/10"
                  >
              ...
              >
                ...
                <div className="flex items-center justify-between py-4 border-y border-slate-100 dark:border-slate-800">
                  <div className="space-y-0.5 max-w-sm">
                    <span className="block text-xs font-bold text-slate-800 dark:text-slate-200">System Dark Mode</span>
                    <p className="text-[10px] text-slate-400 dark:text-slate-500 leading-normal font-semibold">Convert overall workspace containers and backgrounds to deep dark colors.</p>
                  </div>
                  {/* Switch */}
                ...
                <div className="flex items-center justify-between py-2.5">
                  <div className="space-y-0.5 max-w-sm">
                    <span className="block text-xs font-bold text-slate-800 dark:text-slate-200">Default Sidebar view</span>
                    <p className="text-[10px] text-slate-400 dark:text-slate-500 leading-normal font-semibold">Choose whether the sidebar should load as collapsed or expanded on startup.</p>
                  </div>
                  <select 
                    value={sidebarPreference}
                    onChange={(e) => setSidebarPreference(e.target.value)}
                    className="p-2.5 text-xs rounded-xl border border-slate-200 dark:border-slate-850 bg-slate-50/50 dark:bg-slate-950 text-slate-700 dark:text-slate-200 font-bold focus:outline-none focus:border-indigo-500 focus:bg-white dark:focus:bg-slate-900 transition-smooth"
                  >
                    <option value="expanded">Expanded List</option>
                    <option value="collapsed">Collapsed Icons</option>
                  </select>
                </div>
                ...
                <div className="flex items-center justify-between py-4 border-t border-slate-100 dark:border-slate-800">
                  <div className="space-y-0.5 max-w-sm">
                    <span className="block text-xs font-bold text-slate-800 dark:text-slate-200">Compact Dashboard mode</span>
                    <p className="text-[10px] text-slate-400 dark:text-slate-500 leading-normal font-semibold">Shrinks grid paddings and margins for high density data screens.</p>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input 
                      type="checkbox" 
            ...
            {activeTab === 'accessibility' && (
              ...
              >
                <div>
                  <h3 className="font-black text-slate-800 dark:text-slate-100 text-base tracking-tight font-sans">Accessibility Settings</h3>
                  <p className="text-xs font-semibold text-slate-400 dark:text-slate-500 mt-0.5">Customize font scales and contrast variables for comfortable studying</p>
                </div>

                {/* Font Size selector */}
                <div className="flex items-center justify-between py-4 border-y border-slate-100 dark:border-slate-800">
                  <div className="space-y-0.5 max-w-sm">
                    <span className="block text-xs font-bold text-slate-800 dark:text-slate-200">Workspace Font Scale</span>
                    <p className="text-[10px] text-slate-400 dark:text-slate-500 leading-normal font-semibold">Scales text tags for optimal reading legibility.</p>
                  </div>
                  <select 
                    value={fontSize}
                  ...
                  >
                    ...
                    <option value="medium">Medium Default (14px)</option>
                    <option value="large">Large Zoom (16px)</option>
                  </select>
                </div>
                ...
                <div className="flex items-center justify-between py-2.5">
                  <div className="space-y-0.5 max-w-sm">
                    <span className="block text-xs font-bold text-slate-800 dark:text-slate-200">High Contrast details</span>
                    <p className="text-[10px] text-slate-400 dark:text-slate-500 leading-normal font-semibold">Increases textual color saturation and background borders for high legibility.</p>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input 
                      type="checkbox" 
                ...
                <div className="flex items-center justify-between py-4 border-t border-slate-100 dark:border-slate-800">
                  <div className="space-y-0.5 max-w-sm">
                    <span className="block text-xs font-bold text-slate-800 dark:text-slate-200">Reduced Motion transitions</span>
                    <p className="text-[10px] text-slate-400 dark:text-slate-500 leading-normal font-semibold">Disables slide-in animation layers and transitions to decrease processor strain.</p>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input 
                      type="checkbox" 
